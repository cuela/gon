    ...
    <div class="ui threaded comments" id="{{ $comment_item_id }}-comment-0">
        <form class="ui laravelComment-form form" id="{{ $comment_item_id }}-comment-form" data-parent="0" data-item="{{ $comment_item_id }}">
            <div class="field">
                ...
                @if(!Auth::check())
                    <small>Por favor inicie sesión para</small>
                @endif
            </div>
...
?>
    ...
    <button class="ui basic button" id="showComment" data-show-comment="0" data-item-id="{{ $comment_item_id }}">Mostrar Comentarios</button>
</div>
